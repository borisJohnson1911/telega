...
def inlin(c):
    ...
    if c.data == 'mj1':
        ...
        bot.delete_message(chat_id=c.message.chat.id, message_id=c.message.message_id)
        bot.send_message(c.message.chat.id, text="Вы выбрали: Марихуана 1г \nЦена: 1200₽ \nОплата принимается на Яндекс Деньги: 43575638 \nили на Qiwi: 865394865937456 \nКод в комментарии: " + number + "\nПосле оплаты нажмите на кнопку 'Проверить оплату'. Транзакции проверяются вручную. Приверное время подтверждения составляет 15 минут. После того, как оператор удостоверится в успешности оплаты, Вам придет фото, координаты и описание места", reply_markup=check)
